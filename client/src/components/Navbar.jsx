      ...
      <div className="container mx-auto flex flex-wrap p-3  items-center justify-between">
        ...
        <div className='flex gap-3 items-center'>
          <Link className=" border-0 py-1 px-3 rounded bg-gray-100 hover:bg-gray-200   relative " to={"/cart"}>
            <span className='absolute -right-2 -top-2 bg-orange-500 text-white rounded-full px-2 text-md'>{cart.length}</span>
            <i className="ri-shopping-cart-2-line"></i>
          </Link>
          {
            isAuth ? <button className="text-orange-500 bg-gray-100 px-2 py-1 rounded" onClick={handleLogOut}>
            Log Out
          </button>: <Link className=" bg-orange-500 py-1 px-3 hover:bg-orange-600  rounded text-white" to={"/login"}>
            Log in
          </Link>
          }
